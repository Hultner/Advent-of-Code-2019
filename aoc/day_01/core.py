...
def launch_fuel_required(mass: Number):
    "Calculates the fuel required for given mass"
    return int(mass / 3) - 2


def launch_fuel_for_fuel(mass):
    fuel = launch_fuel_required(mass)
    # Base case
    if fuel <= 0:
        return 0
    return fuel + launch_fuel_for_fuel(fuel)


def part_1(puzzle_input=p1):
    return sum(launch_fuel_required(mass) for mass in puzzle_input)


def part_2(puzzle_input=p1):
    """
    --- Part Two ---
    ...
    """
    return sum(launch_fuel_for_fuel(mass) for mass in puzzle_input)
